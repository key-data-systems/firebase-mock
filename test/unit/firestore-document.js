...
describe("MockFirestoreDocument", function() {
  ...
  describe("#update", function() {
    describe("with FieldValue.arrayUnion", function() {
      it("adds new array ", function(done) {
        doc.set({});
        doc.update({ choices: Firestore.FieldValue.arrayUnion("a", "b", "c") });
        doc
          .get()
          .then(function(snap) {
            expect(snap.get("choices")).to.eql(["a", "b", "c"]);
            done();
          })
          .catch(done);
      ...
      it("updates existing nested field", function(done) {
        doc.set({ foo: { choices: ["apple"] } });
        doc.update({
          "foo.choices": Firestore.FieldValue.arrayUnion("orange")
        });

        doc
          .get()
          .then(function(snap) {
            expect(snap.get("foo.choices")).to.eql(["apple", "orange"]);
            done();
          })
          .catch(done);
        db.flush();
      ...
      it("updates existing field", function(done) {
        doc.set({ choices: ["apple"] });
        doc.update({
          choices: Firestore.FieldValue.arrayUnion("orange")
        });

        doc
          .get()
          .then(function(snap) {
            expect(snap.get("choices")).to.eql(["apple", "orange"]);
            done();
          })
          .catch(done);
        db.flush();
